...
<template>
  <div class="space-y-5">
    <div class="flex items-start justify-between gap-3">
      <div>
        ...
        <p class="text-sm text-muted">
          Ce que tu constates run après run. Le guide suggère d’ajuster l’équipe selon ces observations —
          par exemple basculer sur Roigada si tu meurs surtout à des attaques spéciales.
        </p>
      </div>
      <UButton
        v-if="editing === null"
    ...
    <form
      v-if="editing !== null"
      class="p-4 rounded-[var(--ui-radius)] border border-default bg-elevated/30 space-y-3"
      @submit.prevent="save"
    >
    ...
    <div v-if="journal.length" class="space-y-3">
      <article
        ...
        :key="entry.id"
        class="p-4 rounded-[var(--ui-radius)] border border-default space-y-2"
      >
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0">
