...
const phases = computed(() => current.value.content.phases)
const completionGroups = computed(() => current.value.content.completionGroups)
const { completion, byCompletionSection, byPhase } = useProgress()
const { blockersFor } = useNextActions()

/*
 * Les phases sont rendues à part, en tâches et non en objectifs : elles ont un
 * ordre et des `requires`, et elles alimentent « prochaines actions » — ce que
 * la complétion, par construction, ne fait pas.
 *
 * Unbound n'en a plus qu'une (la Battle Frontier, seul reste de sa roadmap) ;
 * un autre jeu peut en avoir plusieurs. On les rend toutes, dans l'ordre.
 */
...
onMounted(() => {
  const firstIncomplete = completionGroups.value.find(
    group => (byCompletionSection.value.get(group.id)?.percent ?? 100) < 100,
  )
  const target = firstIncomplete ?? completionGroups.value[0]
...
<template>
  <div class="space-y-10">
    ...
    <!-- Les phases du jeu, en tâches ordonnées -->
    <SectionBlock
    ...
    >
      <template #action>
        <span class="text-xs text-dimmed whitespace-nowrap tabular-nums">
          {{ byPhase.get(phase.id)?.done ?? 0 }}/{{ phase.tasks.length }}
        </span>
      </template>

      <ContentBlocks v-if="phase.intro" :blocks="phase.intro" compact />
